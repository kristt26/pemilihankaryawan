...
class Jadwal extends BaseController
{
    ...
    public function detail($tanggal)
    {
        ...
        $result['spot'] = $this->jadwal->query("SELECT
            `user`.`fullname`,
            `iklan`.`topik`,
            `jadwalsiaran`.`tanggal`,
            `jadwalsiaran`.`waktu`,
            `layanan`.`layanan`,
            `tarif`.`kategori`,
            `tarif`.`jenis`
        FROM
            ...
            LEFT JOIN `layanan` ON `layanan`.`id` = `tarif`.`layananid` WHERE jadwalsiaran.tanggal='$tanggal' AND 
            layanan.layanan='Spot Iklan' ORDER BY tarif.kategori, tarif.jenis, jadwalsiaran.waktu")->getResultArray();
        $result['pengumuman'] = $this->jadwal->query("SELECT
            `user`.`fullname`,
            `iklan`.`topik`,
            `jadwalsiaran`.`tanggal`,
            `jadwalsiaran`.`waktu`,
        ...
        FROM
            ...
            LEFT JOIN `tarif` ON `tarif`.`id` = `iklan`.`tarifid`
            LEFT JOIN `user` ON `user`.`id` = `iklan`.`userid`
            LEFT JOIN `layanan` ON `layanan`.`id` = `tarif`.`layananid` WHERE jadwalsiaran.tanggal='$tanggal' AND layanan.layanan='Pengumuman' 
            ORDER BY tarif.kategori, jadwalsiaran.waktu")->getResultArray();
        return $this->respond($result);
    }
}
